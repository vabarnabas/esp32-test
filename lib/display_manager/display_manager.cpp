...
static void drawMenu(const MenuView& v) {
  ...
  int maxRows = (display.height() - TOP) / ROW_H;   // ~4 rows fit
  int first = 0;
  if (v.selectedItem >= maxRows) first = v.selectedItem - maxRows + 1;   // scroll to keep selected visible
...
void renderEInk(const MenuView& v) {
  ...
  if (full) { display.setFullWindow(); partialCount = 0; }
  else      { display.setPartialWindow(0, 0, display.width(), display.height()); partialCount++; }

  display.firstPage();
  do { drawMenu(v); } while (display.nextPage());
  display.hibernate();
}